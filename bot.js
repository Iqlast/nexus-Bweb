...
// Menyensor sebagian besar private key
function maskPrivateKey(privateKey) {
    const visibleChars = 6;
    return privateKey.slice(0, visibleChars) + '*'.repeat(privateKey.length - visibleChars);
}
...
async function getTask(wallet, axiosInstance) {
    try {
        ...
        console.log(`Task berhasil didapatkan untuk ${wallet.address}`);
        console.log('Respon Server:', response.data);
        return response.data;
    } catch (error) {
        console.error(`Gagal mendapatkan task untuk ${wallet.address}:`, error.message);
        return null;
    }
}
...
async function submitTask(wallet, axiosInstance) {
    try {
        ...
        console.log(`Task berhasil disubmit untuk ${wallet.address}`);
        console.log('Respon Server:', response.data);
        return response.data;
    } catch (error) {
        console.error(`Gagal submit task untuk ${wallet.address}:`, error.message);
        return null;
    }
}
